...
def plot_chart(returns, sample_start, sample_end, long_short, factor_name=''):
    if long_short == 'Yes':
        (1+returns.iloc[:,-2]).cumprod().plot(figsize=(13,8), fontsize=13)
        plt.title('Fixed Income' + str(factor_name)+ ' Factor Long Only In-Sample Backtest: '+ str(sample_start) +' -' + str(sample_end), fontsize=14)
        plt.legend(loc=(1,0.1))
    else:
        (1+returns).cumprod().plot(figsize=(13,8), fontsize=13),
        plt.title('Fixed Income ' + str(factor_name)+ ' Factor Long Only In-Sample Backtest: '+ str(sample_start) +' -' + str(sample_end), fontsize=14)
        plt.legend(loc=(1,0.1))